...
const T_FB = {
  ...
  searching: 'Searching...',
  found: 'Rival found!',
  disconnected: 'Rival disconnected',
  pressFind: 'Press Find Match',
  youWin: 'YOU WIN',
  youLose: 'YOU LOSE',
  draw: 'DRAW',
  score: 'Score',
...
async function startVoice() {  if (!room || ls === null) return
  mm = pm = false
...
function joinLobby() {
  room = joinRoom(CFG, LOBBY)  log('Lobby — me: ' + selfId.slice(0, 6))
  ...
  getHash(({selfH, peerH, t}, from) => {
    ...
    if (!ok) {      log('HACK DETECTED at tick ' + t)
...
function scheduleStart(ts) {
  ...
  elGame.classList.remove('hidden')  resizeCanvas()
  startVoice()
...
function showEnd(won, msg) {
  ...
  elOver.classList.remove('hidden')  elOver.classList.remove('win')
